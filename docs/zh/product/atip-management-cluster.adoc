...
. **xref:mgmt-cluster-image-preparation-airgap[隔离环境的映像准备]**：此步骤旨在展示准备隔离场景中所用清单和文件的差异。
  ...
  - xref:mgmt-cluster-helm-values-folder-airgap[helm values 文件夹中的修改]：必须修改 `helm/values` 文件夹以包含在隔离环境中运行管理集群所需的配置。
...
目录结构的示例可以在 https://github.com/suse-edge/telco-cloud-examples[SUSE Edge GitHub 储存库下的“telco-examples”文件夹]中找到。
====
...
- `server.yaml`：默认情况下，安装的 `CNI` 插件是 `Cilium`，因此您无需创建此文件夹和文件。如果您需要自定义 `CNI` 插件，可以使用 `kubernetes/config` 文件夹下的 `server.yaml` 文件。它包含以下信息：
+
[,yaml]
----
cni:
...
- `longhorn.yaml`：包含创建 `Longhorn` 组件的配置（无需修改）。
+
[,yaml]
----
global:
...
- 将上一个 `metal3.yaml` 文件中的 `true` 设置为 `additionalTrustedCAs`，以启用来自外部媒体服务器的额外受信任 CA。
...
`additional_ca_cert` 是外部媒体服务器的 base64 编码 CA 证书。您可以使用以下命令手动对证书进行编码并生成密钥：
...
- 必须修改 `mgmt-cluster.yaml` 文件以包含 `rancher-turtles-airgap-resources` helm chart。
...
还必须添加 `rancher-turtles-airgap-resources` helm chart，这会创建如 https://documentation.suse.com/cloudnative/cluster-api/{rancher-turtles-docs-version}/en/operator/airgapped.html[Rancher Turtles 隔离文档] 中所述的资源。 这也需要一个用于 rancher-turtles chart 的 turtles.yaml values 文件，以指定必要的配置。
...
- `turtles.yaml`：包含指定 Rancher Turtles 隔离操作所需的配置，请注意这取决于 rancher-turtles-airgap-resources chart 的安装。
+
[,yaml]
----
