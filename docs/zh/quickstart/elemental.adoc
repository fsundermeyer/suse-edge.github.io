...
本节记录了作为 SUSE Edge 一部分的“自动回传网络配置”解决方案，我们使用 Elemental 来协助节点注册。Elemental 是一个软件栈，支持远程主机注册以及通过 Kubernetes 进行集中的云原生操作系统管理。在 SUSE Edge 栈中，我们使用 Elemental 的注册功能来实现远程主机注册至 Rancher，以便将主机集成到集中式管理平台中，并从那里统一部署和管理 Kubernetes 集群以及分层组件、应用程序及其生命周期。
