...
SUSE Edge Virtualization 支持两种运行虚拟机的方法：
...
本指南不会引导您完成 Kubernetes 的部署，但它假设您已经安装了 SUSE Edge 对应版本的 https://k3s.io/[K3s] 或 https://docs.rke2.io/install/quickstart[RKE2]，并且您已相应地配置了 kubeconfig，以便可以以超级用户身份执行标准的 `kubectl` 命令。我们假设您的节点构成一个单节点集群，尽管对于多节点部署，预计不会有重大差异。

SUSE Edge Virtualization 通过三个独立的 Helm 图表 进行部署，具体如下：
...
这些 Helm 图表中的每一个都根据您当前使用的 SUSE Edge 版本进行版本管理。对于生产/受支持的使用场景，请使用可在 SUSE Registry 中找到的制品。
...
在本节中，我们将展示如何将虚拟机作为标准 Kubernetes 服务公开，并通过 Kubernetes Ingress 服务使其可用，例如 https://docs.rke2.io/networking/networking_services#nginx-ingress-controller[NGINX with RKE2] 或 https://docs.k3s.io/networking/networking-services#traefik-ingress-controller[Traefik with K3s]。本文档假设这些组件已正确配置，并且您拥有适当的 DNS 指针（例如通过通配符），指向您的 Kubernetes 服务器节点或入口虚拟 IP，以便进行正确的入口解析。

> NOTE: 在 SUSE Edge 3.1+ 版本中，如果您在多服务器节点配置中使用 K3s，可能需要为 Ingress 配置基于 MetalLB 的 VIP；RKE2 则不需要这样做。
...
SUSE Edge 正在使用 <<components-eib>> 来定制基础 SUSE® Linux Micro 镜像。
