...
[NOTE]
====
`Upgrade Controller`目前仅支持针对*非隔离的管理*集群的 SUSE Edge 平台升级。有关详细信息，请参见<<components-upgrade-controller-known-issues>>部分。
====

== SUSE Edge 如何使用 Upgrade Controller？

*Upgrade Controller*对于自动化（以前需要手动执行的）将管理集群从一个 SUSE Edge 发布版本升级到下一个版本所需的“第 2 天”操作至关重要。
...
图表将根据其在 `ReleaseManifest` 中的顺序依次升级。还可以通过 `UpgradePlan` 传递其他值。如果图表的版本在新的 SUSE Edge 发行版中保持不变，则不会对其进行升级。有关更多信息，请参见<<components-upgrade-controller-extensions-upgrade-plan>>。
