...
扩展允许用户、开发人员、合作伙伴和客户扩展和增强Rancher UI。SUSE Edge 提供 KubeVirt 仪表板扩展。

有关 Rancher 仪表板扩展的一般信息，请参阅 `{link-rancher-extensions}[Rancher documentation]`。

== 安装

所有 SUSE Edge {version-edge} 组件（包括仪表板扩展）均作为 OCI 制品分发。要安装 SUSE Edge 扩展，您可以使用 Rancher 仪表板 UI、Helm 或 Fleet：
...
+
SUSE Edge Helm charts 储存库 URL：`oci://registry.suse.com/edge/charts`
+
