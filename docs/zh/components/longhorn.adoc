...
* 至少安装有一台 SUSE Linux Micro {version-sl-micro} 的主机；可以是物理机或虚拟机
...
----
+
. 在 `longhorn-system` 名称空间中安装 SUSE Storage：
+
...
如果您使用 kubectl 或 Helm 安装了 Longhorn，则需要设置 Ingress 控制器以允许外部流量进入集群。默认情况下未启用身份验证。如果使用了 Rancher 应用合集，Rancher 会自动创建一个带有访问控制（rancher-proxy）的 Ingress 控制器。
...
我们将演示如何执行此操作，以便在其上部署带有 Longhorn 的 RKE2 集群。
