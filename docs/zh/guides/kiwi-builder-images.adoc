...
此处理利用 https://osinside.github.io/kiwi/[Kiwi] 来运行镜像构建。SUSE Edge 附带了一个容器化版本，通过内置的辅助工具简化了整个处理，允许指定所需的 *控制文件*。控制文件定义了所需的输出镜像类型，常见类型如下：
...
有关更多详细信息，请参阅 https://documentation.suse.com/sle-micro/{version-sl-micro}/html/Micro-deployment-images/index.html#alp-images-installer-type[SUSE Linux Micro {version-sl-micro}] 文档。

此处理适用于 {x86-64} 和 {aarch64} 架构，尽管并非所有镜像控制文件都适用于这两种架构，例如在 SUSE Edge {version-edge-registry} 中，当使用 SUSE Linux Micro {version-sl-micro} 时，带有实时内核的控制文件（即“Base-RT”或“Base-RT-SelfInstall”）目前不适用于 {aarch64}。

NOTE: 必须使用与所构建镜像架构相同的构建主机。换句话说，要构建 {aarch64} 镜像，需要使用 {aarch64} 构建主机，反之亦然（对于 {x86-64}），目前不支持交叉构建。

== 先决条件

Kiwi 镜像构建器需要满足以下条件：

* 一台与所构建镜像架构相同的 SUSE Linux Micro {version-sl-micro} 主机（“构建系统”）。
...
由于某些限制，目前需要禁用 SELinux。连接到 SUSE Linux Micro {version-sl-micro} 镜像构建主机并确保已禁用 SELinux：
