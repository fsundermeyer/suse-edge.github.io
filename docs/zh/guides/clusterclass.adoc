...
使用 Cluster API (CAPI) 和 RKE2 提供程序部署 Kubernetes 集群需要定义多个自定义资源。
...
* **Metal3MachineTemplate**:此资源充当创建构成 Kubernetes 集群工作节点的各个计算实例的蓝图，定义了要使用的镜像。
...
在以下 ClusterClass 示例中，定义了一些变量，这些变量将在集群实例化过程中使用实际值进行替换。
示例中使用了以下变量：
...
ClusterClass 定义文件基于以下 3 个资源进行定义：
...
之前在模板（ClusterClass 定义文件）中定义的变量将替换为该集群实例化的最终值：
