...
== Uso do Rancher no SUSE Edge

O Rancher fornece várias funcionalidades essenciais para a pilha SUSE Edge:
...
O catálogo de aplicativos integrado do Rancher pode simplificar a implantação e o gerenciamento de aplicativos conteinerizados em clusters SUSE Edge, permitindo a implantação sem interrupções de cargas de trabalho de borda.

=== Segurança e aplicação de políticas

O Rancher fornece ferramentas de governança baseadas em políticas, controle de acesso com base em função (RBAC) e integração com provedores de autenticação externos. Isso ajuda as implantações do SUSE Edge a manter a segurança e a conformidade, o que é fundamental em ambientes distribuídos.
