...
O SUSE Edge Virtualization suporta dois métodos de execução de máquinas virtuais:
...
Este guia não o conduzirá pela implantação do Kubernetes, mas pressupõe que você tenha instalado a versão apropriada para SUSE Edge de https://k3s.io/[K3s] ou https://docs.rke2.io/install/quickstart[RKE2] e que seu kubeconfig esteja configurado adequadamente para que os comandos padrão `kubectl` possam ser executados como superusuário. Presumimos que seu nó forma um cluster de nó único, embora não sejam esperadas diferenças significativas para implantação de vários nós.

O SUSE Edge Virtualization é implantado por meio de três charts Helm separados, especificamente:
...
Cada um desses charts Helm é versionado de acordo com a versão do SUSE Edge que você está usando atualmente. Para uso em produção/suportado, empregue os artefatos que podem ser encontrados no Registro SUSE.
...
O SUSE Edge Virtualization fornece uma extensão para o Rancher Manager, permitindo o gerenciamento básico de máquinas virtuais usando a interface do Rancher.
...
O SUSE Edge está usando <<components-eib>> para personalizar imagens base do SUSE Linux Micro OS.
