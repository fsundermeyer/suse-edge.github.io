...
== Como o SUSE Edge usa o Elemental?
...
NOTE: No SUSE Edge {version-edge} nós *não* utilizamos os aspectos de gerenciamento do sistema operacional do Elemental e, portanto, não é possível gerenciar o patch do seu sistema operacional via Rancher. Em vez de usar as ferramentas do Elemental para criar imagens de implantação, o SUSE Edge usa o Edge Image Builder, que consome a configuração de registro.

== Melhores práticas

=== Mídia de instalação

A maneira recomendada pelo SUSE Edge para criar imagens de implantação que possam aproveitar o Elemental para registro no Rancher, no modelo de implantação 'phone home network provisioning', é seguir as instruções detalhadas no <<quickstart-elemental,remote host onboarding with Elemental>> quickstart.
