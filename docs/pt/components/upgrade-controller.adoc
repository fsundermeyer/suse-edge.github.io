...
Um controlador Kubernetes capaz de fazer upgrade nos seguintes componentes da plataforma SUSE Edge:
...
[NOTE]
====
O `Upgrade Controller` atualmente suporta fazer upgrade da plataforma SUSE Edge apenas para clusters *de gerenciamento não air-gapped*. Consulte a seção <<components-upgrade-controller-known-issues>> para obter mais informações.
====

== Como o SUSE Edge usa o Upgrade Controller?

O *Upgrade Controller* é essencial na automação das operações de "Dia 2" (anteriormente manuais) necessárias para fazer upgrade de clusters de gerenciamento de uma versão de lançamento do SUSE Edge para a próxima.
