...
Um controlador Kubernetes capaz de fazer upgrade dos seguintes componentes da plataforma SUSE Edge:
...
[NOTE]
====
O `Upgrade Controller` atualmente suporta upgrades da plataforma SUSE Edge apenas para clusters *de gerenciamento não air-gapped*. Consulte a seção <<components-upgrade-controller-known-issues>> para obter mais informações.
====
