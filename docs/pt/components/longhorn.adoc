...
* Pelo menos um host com o SUSE Linux Micro {version-sl-micro} instalado; ele pode ser físico ou virtual.
...
. Adicione o repositório Helm de Charts do Rancher:
+
[,shell]
----
...
----
+
. Busque os charts mais recentes do repositório:
+
[,shell]
----
helm repo update
----
+
. Instale o SUSE Storage no namespace `longhorn-system`:
+
...
O SUSE Edge está usando o <<components-eib>> para personalizar imagens base do sistema operacional SUSE Linux Micro.
Vamos demonstrar como fazer isso para provisionar um cluster RKE2 com o Longhorn sobre ele.
...
Certifique-se de que o Longhorn tenha sido implantado com sucesso:
