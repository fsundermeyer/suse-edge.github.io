...
Embora a versão atual do Elemental tenha uma maneira de criar sua própria mídia de instalação, no SUSE Edge {version-edge} fazemos isso com o Kiwi e o Edge Image Builder, para que o sistema resultante seja criado com o https://www.suse.com/products/micro/[SUSE Linux Micro] como o Sistema Operacional base.
...
No SUSE Edge 3.1, o Operador Elemental coloca um marcador no sistema operacional que acionará automaticamente o processo de limpeza; ele irá parar todos os serviços do Kubernetes, remover todos os dados persistentes, desinstalar todos os serviços do Kubernetes, limpar quaisquer diretórios restantes do Kubernetes/Rancher e forçar um novo registro no Rancher via a configuração original do Elemental `MachineRegistration`. Isso acontece automaticamente, não há necessidade de qualquer intervenção manual. O script que é chamado pode ser encontrado em `/opt/edge/elemental_node_cleanup.sh` e é acionado via `systemd.path` após a colocação do marcador, portanto sua execução é imediata.
