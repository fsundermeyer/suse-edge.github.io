...
Assim que a lista for preenchida, use o filtro para mostrar apenas "Micro {version-sl-micro}".
Marque a caixa para SUSE Linux Micro {version-sl-micro} para a arquitetura de hardware na qual seus nós de borda serão executados (`x86_64` ou `aarch64`).
...
Como "Canal Filho", selecione o controle deslizante "incluir recomendados" para a arquitetura de hardware para a qual sua chave de ativação é usada. Isso adicionará o canal "SUSE-Manager-Tools-For-SL-Micro-{version-sl-micro}".
...
Para uma configuração de laboratório mínima, podemos usar a mesma máquina virtual na qual o SUSE Multi-Linux Manager Server está sendo executado. Certifique-se de que você tenha espaço em disco suficiente na máquina virtual! Esta não é uma configuração recomendada para uso em produção. Consulte <<eib-prerequisites>> para ver os sistemas operacionais host com os quais testamos o Edge Image Builder.
...
Neste exemplo, a senha de root seria \"root\". Consulte <<configuring-os-users>> para criar hashes de senha para a senha segura que você deseja usar.
