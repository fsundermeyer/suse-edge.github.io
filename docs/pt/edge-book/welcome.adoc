...
= Documentação do SUSE Edge {version-edge}
:revdate: 2025-07-15
:revnumber: 3.5
...
* *Gerenciamento*: Esta é a parte centralizada do SUSE Edge que é usada para gerenciar o provisionamento e o ciclo de vida dos clusters downstream conectados. O cluster de gerenciamento normalmente inclui os seguintes componentes:
  ...
  ** Um controlador dedicado de <<components-upgrade-controller,Gerenciamento de Ciclo de Vida>> que lida com fazer upgrade dos componentes do cluster de gerenciamento para uma determinada versão do SUSE Edge.
...
* *Downstream*: Esta é a parte distribuída do SUSE Edge que é usada para executar as cargas de trabalho dos usuários na borda, ou seja, o software que é executado no próprio local de borda, e é tipicamente composto pelos seguintes componentes:
...
Esta solução oferece uma grande flexibilidade e consistência na maneira como os sistemas são provisionados e integrados, independentemente de sua localização, tipo ou especificação de sistema, ou de quando são ligados pela primeira vez. O SUSE Edge permite total flexibilidade e personalização do sistema por meio do Edge Image Builder e aproveita os recursos de registro da oferta Elemental do Rancher para integração de nós e provisionamento de Kubernetes, juntamente com o SUSE Multi-Linux Manager para patch do sistema operacional. O início rápido para esta solução pode ser encontrado em <<quickstart-elemental>>.
...
A lista completa de componentes, juntamente com um link para uma descrição de alto nível de cada componente e como ele é usado no SUSE Edge, pode ser encontrada abaixo:
