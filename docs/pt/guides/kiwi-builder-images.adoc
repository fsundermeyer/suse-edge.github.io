...
Consulte a documentação do https://documentation.suse.com/sle-micro/{version-sl-micro}/html/Micro-deployment-images/index.html#alp-images-installer-type[SUSE Linux Micro {version-sl-micro}] para obter mais detalhes.

Este processo funciona para as arquiteturas {x86-64} e {aarch64}, embora nem todos os perfis de imagem estejam disponíveis para ambas as arquiteturas, por exemplo, no SUSE Edge {version-edge-registry}, onde o SUSE Linux Micro {version-sl-micro} é usado, um perfil com um kernel de tempo real (ou seja, \"Base-RT\" ou \"Base-RT-SelfInstall\") não está disponível atualmente para {aarch64}.

NOTE: É necessário usar um host de compilação com a mesma arquitetura das imagens que estão sendo criadas. Em outras palavras, para criar uma imagem {aarch64}, é necessário usar um host de compilação {aarch64}, e vice-versa para {x86-64} - compilações cruzadas não são suportadas no momento.

== Pré-requisitos

O construtor de imagens Kiwi requer o seguinte:

* Um host SUSE Linux Micro {version-sl-micro} ("sistema de compilação") com a mesma arquitetura da imagem sendo compilada.
...
Devido a certas limitações, é necessário atualmente desativar o SELinux. Conecte-se ao host de compilação da imagem SUSE Linux Micro {version-sl-micro} e certifique-se de que o SELinux esteja desativado:
