...
* *Problemas de combustão*: O EIB depende da combustão para a personalização do sistema operacional e a implantação de todos os outros componentes do SUSE Edge. Isso também inclui scripts personalizados sendo colocados na pasta custom/scripts. Observe que o processo de combustão está sendo executado no momento `initrd`, portanto, o sistema não está completamente inicializado quando os scripts são executados.
...
* *Logs do contêiner de compilação*: Verifique os logs do contêiner de build. Os logs são gerados no diretório que foi usado para armazenar os artefatos. Verifique `docker logs` ou `podman logs` para obter as informações necessárias também.
+
[NOTE]
====
