...
* *ネットワークインタフェース名の不一致*:`net.ifnames=1`カーネル引数は https://documentation.suse.com/smart/network/html/network-interface-predictable-naming/index.html[ネットワークインタフェースの予測可能な命名スキーム]を有効にするため、`eth0`は存在せず、代わりに`enp2s0`のような他の命名スキームが使用されます。
