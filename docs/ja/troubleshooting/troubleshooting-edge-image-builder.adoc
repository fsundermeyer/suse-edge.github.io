...
EIBはカスタムSUSE Edgeイメージを作成するために使用されます。
...
* *Combustionの問題*:EIBは、OSのカスタマイズおよびその他のSUSE Edgeコンポーネントのデプロイメントを、Combustionに依存しています。これには、custom/scriptsフォルダーに配置されるカスタムスクリプトも含まれます。combustion処理は`initrd`時に実行されるため、スクリプトが実行される時点ではシステムは完全に起動していないことに注意してください。
