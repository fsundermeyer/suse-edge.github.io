...
このプロセスでは、 https://osinside.github.io/kiwi/[Kiwi]を使用してイメージビルドを実行します。SUSE Edgeには、ヘルパーユーティリティが組み込まれたコンテナ化バージョンが付属しており、全体的なプロセスを簡素化し、必要なターゲット*プロファイル*を指定できます。プロファイルは、必要な出力イメージのタイプを定義します。一般的なものを以下に示します。
...
詳細については、 https://documentation.suse.com/sle-micro/{version-sl-micro}/html/Micro-deployment-images/index.html#alp-images-installer-type[SUSE Linux Micro {version-sl-micro}]のドキュメントを参照してください。

このプロセスは{x86-64}と{aarch64}の両方のアーキテクチャで機能しますが、すべてのイメージプロファイルが両方のアーキテクチャで利用できるわけではありません。たとえば、SUSE Linux Micro {version-sl-micro}が使用されるSUSE Edge {version-edge-registry}では、リアルタイムカーネル（つまり、「Base-RT」または「Base-RT-SelfInstall」）を含むプロファイルは、現在では利用できません。「Base-RT」または「Base-RT-SelfInstall」は、現在{aarch64}では利用できません。

NOTE: ビルドするイメージと同じアーキテクチャのビルドホストを使用する必要があります。言い換えると、{aarch64}イメージを構築するには{aarch64}ビルドホストを使用する必要があり、{x86-64}の場合も同様です。現時点ではクロスビルドはサポートされていません。

== 前提条件

Kiwiイメージビルダーには以下が必要です。

* 構築するイメージと同じアーキテクチャのSUSE Linux Micro {version-sl-micro}ホスト（「ビルドシステム」）が必要です。
...
特定の制限により、現在はSELinuxを無効にする必要があります。SUSE Linux Micro {version-sl-micro} イメージビルドホストに接続し、SELinuxが無効になっていることを確認してください。
