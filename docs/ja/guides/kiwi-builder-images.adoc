...
このプロセスでは、 https://osinside.github.io/kiwi/[Kiwi]を使用してイメージのビルドを実行します。SUSE Edgeには、ヘルパーユーティリティを組み込んだコンテナ化されたバージョンが含まれており、全体のプロセスが簡素化され、必要な*プロファイル*を指定できるようになっています。プロファイルは、必要な出力イメージのタイプを定義します。一般的なものを以下に示します。
