...
次の例では、既存のイメージ定義を使用してKubernetes構成を指定します（この例ではシステムとその役割がリストされていないため、デフォルトで単一ノードと想定します）。これにより、EIBは単一ノードのRKE2 Kubernetesクラスターをプロビジョニングするように指示されます。ユーザー提供のワークロード（マニフェスト経由）および階層化されたコンポーネント（Helm経由）の両方のデプロイ自動化を示すために、SUSE Edge Helmチャートを使ってKubeVirtをインストールし、Kubernetesマニフェストを使ってNGINXをインストールします。既存のイメージ定義に追加する必要がある追加の構成は次のとおりです。
