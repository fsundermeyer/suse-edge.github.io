...
`{micro-base-image-raw}` OS イメージファイルは、 https://scc.suse.com/[SUSE Customer Center] または https://www.suse.com/download/sle-micro/[SUSE Download page] からダウンロードする必要があります。
...
. Rancher Turtles を介して CAPI の依存関係をインストールします。
...
永続ストレージプロバイダーをインストールする必要があります。SUSE Storage が推奨されますが、dev/PoC 環境では `local-path-provisioner` を使用することもできます。以下の手順では、StorageClass が https://kubernetes.io/docs/tasks/administer-cluster/change-default-storage-class/[デフォルトとしてマーク] されていることを前提としています。そうでない場合は、Metal^3^ チャートの追加設定が必要です。
...
=== クラスター API の依存関係のインストール

クラスター API の依存関係は、Rancher Turtles Helm チャートを介して管理されます:
...
しばらくすると、コントローラーポッドが `capi-system`、`capm3-system`、`rke2-bootstrap-system`、および `rke2-control-plane-system` ネームスペースで実行されるはずです。
...
[NOTE]
====
`net.ifnames=1` は https://documentation.suse.com/smart/network/html/network-interface-predictable-naming/index.html[ネットワークインタフェースの予測可能な命名] を有効にします。

これは Metal^3^ チャートのデフォルト設定と一致しますが、この設定は構成されたチャートの `predictableNicNames` 値と一致している必要があります。

また、`ignition.platform.id=openstack` は必須であることに注意してください。この引数がないと、Metal^3^ 自動フローでの ignition を介した SUSE Linux Micro の設定は失敗します。
...
* 特にデバイスの予測可能な名前を設定しようとする場合、ironic python agent (IPA) とターゲットオペレーティングシステム (SL Micro 6.0/6.1) の間でネットワークデバイス名の不一致が発生する可能性があります。

これは、ironic python agent (IPA) のカーネルが現在ターゲットオペレーティングシステム (SL Micro 6.0/6.1) のカーネルと一致していないために発生します。そのため、ネットワークドライバーに不一致が生じ、IPA が SL Micro の想定とは異なる命名パターンでネットワークデバイスを検出してしまいます。

当面の回避策として、2つの異なるアプローチがあります。 +
* ネットワーク設定を含む2つの異なるシークレットを作成します。1つはIPAが検出するデバイス名を使用してIPAで使用し、`preprovisioningNetworkDataName`定義で`BareMetalHost`として使用します。もう1つはSL Microが検出するデバイス名を使用して、`networkData.name`定義で`BareMetalHost`として使用します。 +
* 代わりにUUIDを使用して、生成されたnmconnectionファイル上の他のインターフェースを参照します。
詳細は link:..tips/metal3.adoc[tips and tricks] セクションを参照してください。
...
<<atip, SUSE Telco Cloud Documentation>> には、通信事業者向けユースケースにおける Metal^3^ のより高度な使用例が記載されています。
