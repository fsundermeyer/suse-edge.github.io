...
TIP: 管理クラスタがシングルノードである場合は、MetalLB を介して管理される追加のフローティング IP の要件を回避できます。を参照してください。<<metal3-single-node-configuration>>
...
<<atip, SUSE Telco Cloud Documentation>>には、通信事業者向けユースケースにおけるMetal^3^のより高度な使用例が記載されています。
