...
NOTE: SUSE Edge {version-edge}では、Elementalのオペレーティングシステム管理の側面は活用して*いない*ため、Rancherを介してオペレーティングシステムのパッチ適用を管理することはできません。デプロイメントイメージの構築にElementalツールを使用する代わりに、SUSE Edgeは登録設定を取り込むEdge Image Builderツールを使用します。
