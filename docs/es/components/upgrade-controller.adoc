...
Un controlador de Kubernetes capaz de actualizar versión en los siguientes componentes de la plataforma SUSE Edge:
...
[NOTE]
====
El `Upgrade Controller` actualmente admite actualizaciones de la plataforma SUSE Edge únicamente para *clústeres de gestión no aislados*. Consulte la sección <<components-upgrade-controller-known-issues>> para obtener más información.
====

== ¿Cómo utiliza SUSE Edge el Upgrade Controller?

El *Upgrade Controller* es esencial para automatizar las operaciones de «Día 2» (anteriormente manuales) necesarias para actualizar los clústeres de gestión de una versión de lanzamiento de SUSE Edge a la siguiente.
...
Como alternativa a la instalación manual descrita anteriormente, es posible instalar el Upgrade Controller como parte del despliegue inicial orquestado por <<components-eib, Edge Image Builder>>.
...
Los gráficos se actualizarán secuencialmente según su orden en el `ReleaseManifest`. También se pueden pasar valores adicionales a través del `UpgradePlan`. Si la versión de un chart permanece sin cambios en la nueva versión de SUSE Edge, no se actualizará. Para obtener más información acerca de esto, consulte <<components-upgrade-controller-extensions-upgrade-plan>>.
...
* El Upgrade Controller espera que cualquier chart de Helm adicional de SUSE Edge que se despliegue a través de <<components-eib,EIB>> tenga su link:https://docs.rke2.io/helm#using-the-helm-crd[HelmChart CR] desplegado en el espacio de nombres `kube-system`. Para hacer esto, configure la propiedad `installationNamespace` en su archivo de definición de EIB. Para obtener más información, consulte la link:https://github.com/suse-edge/edge-image-builder/blob/main/docs/building-images.md#kubernetes[documentación en sentido ascendente].
