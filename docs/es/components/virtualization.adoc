...
Esta guía no le explicará el despliegue de Kubernetes, pero asume que ha instalado la versión de https://k3s.io/[K3s] o https://docs.rke2.io/install/quickstart[RKE2] adecuada para SUSE Edge, y que tiene su kubeconfig configurado en consecuencia para que los comandos estándar de `kubectl` se puedan ejecutar como superusuario. Asumimos que su nodo forma un clúster de un solo nodo, aunque no se esperan diferencias significativas para los despliegues de varios nodos.

SUSE Edge Virtualization se despliega mediante tres charts de Helm independientes, concretamente:
...
Cada uno de estos charts de Helm está versionado de acuerdo con la versión de SUSE Edge que está utilizando actualmente. Para un uso en producción o con soporte, emplee los artefactos que se pueden encontrar en el registro de SUSE.
...
En esta sección, mostramos cómo puede exponer máquinas virtuales como servicios estándar de Kubernetes y hacerlas disponibles a través del servicio Ingress de Kubernetes, por ejemplo, https://docs.rke2.io/networking/networking_services#nginx-ingress-controller[NGINX con RKE2] o https://docs.k3s.io/networking/networking-services#traefik-ingress-controller[Traefik con K3s]. Este documento asume que estos componentes ya están configurados adecuadamente y que tienes un puntero DNS apropiado, por ejemplo, mediante un carácter comodín, para apuntar a tus nodos de servidor de Kubernetes o a tu IP virtual de ingress para una adecuada resolución de ingress.

> NOTE: En SUSE Edge 3.1+, si está usando K3s en una configuración de nodo multiservidor, es posible que haya necesitado configurar una VIP basada en MetalLB para Ingress; esto no es necesario para RKE2.
...
SUSE Edge Virtualization proporciona una extensión para Rancher Manager, que permite la gestión básica de máquinas virtuales mediante la interfaz de usuario del panel de control de Rancher.
