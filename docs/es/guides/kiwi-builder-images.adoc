...
Consulte la documentación de https://documentation.suse.com/sle-micro/{version-sl-micro}/html/Micro-deployment-images/index.html#alp-images-installer-type[SUSE Linux Micro {version-sl-micro}] para obtener más detalles.

Este proceso funciona tanto para arquitecturas {x86-64} como {aarch64}, aunque no todos los perfiles de imagen están disponibles para ambas arquitecturas, por ejemplo, en SUSE Edge {version-edge-registry}, donde se utiliza SUSE Linux Micro {version-sl-micro}, un perfil con un núcleo de Linux en tiempo real (es decir, \"Base-RT\" o \"Base-RT-SelfInstall\") no está disponible actualmente para {aarch64}.

NOTE: Es necesario utilizar un host de compilación con la misma arquitectura que las imágenes que se están compilando. En otras palabras, para crear una imagen {aarch64}, es necesario utilizar un host de compilación {aarch64}, y viceversa para {x86-64}; las compilaciones cruzadas no son compatibles en este momento.

== Requisitos previos

El generador de imágenes Kiwi requiere lo siguiente:

* Un host de SUSE Linux Micro {version-sl-micro} (\"sistema de compilación\") con la misma arquitectura que la imagen que se está compilando.
...
Debido a ciertas limitaciones, actualmente es necesario desactivar SELinux. Conéctese al host de compilación de imágenes de SUSE Linux Micro {version-sl-micro} y asegúrese de que SELinux esté desactivado:
