...
SUSE Multi-Linux Manager se incluye en SUSE Edge para proporcionar automatización y control que garanticen que SUSE Linux Micro, como sistema operativo subyacente, se mantenga constantemente actualizado en todos los nodos de su ampliación edge. También se puede utilizar para gestionar Kubernetes y las aplicaciones desplegadas en Kubernetes en vuestros edge nodes.
...
Una vez que la lista esté completa, utilice el filtro para mostrar solo «Micro {version-sl-micro}».
Marque la casilla de SUSE Linux Micro {version-sl-micro} para la arquitectura de hardware en la que se ejecutarán los nodos edge (`x86_64` o `aarch64`).
...
Como «Canal secundario», seleccionad el control deslizante «incluir recomendado» para la arquitectura de hardware para la que se utiliza vuestra clave de activación. Esto añadirá el canal «SUSE-Manager-Tools-For-SL-Micro-{version-sl-micro}».
...
Descargue o copie la imagen `{micro-default-image-iso}` en el directorio `base-images` y llámela «slemicro.iso».

La creación de imágenes {aarch64} en un host de compilación basado en la familia de arquitecturas ARM es una vista previa tecnológica en SUSE Edge {version-edge}. Lo más probable es que funcione, pero aún no es compatible. Si queréis probarlo, debéis ejecutar Podman en una máquina Arm de 64 bits y debéis sustituir «x86_64» en todos los ejemplos y fragmentos de código por «aarch64».
...
Aquí tiene un ejemplo sencillo que instala SL Micro {version-sl-micro}, establece una contraseña de root y el mapa de teclas, inicia la interfaz gráfica de usuario Cockpit y registra su nodo en SUSE Multi-Linux Manager:
...
En este ejemplo, la contraseña de root sería «root». Consulte <<configuring-os-users>> para crear hashes de contraseña para la contraseña segura que quiera utilizar.
